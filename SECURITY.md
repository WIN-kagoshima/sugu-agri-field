...
Email `security@sugukuru.dev` (placeholder — replace with your real reporting address before publishing) with:
...
- Token store: when running with the URL-mode elicitation flow enabled, set `SUGU_TOKEN_ENC_KEY` (32-byte base64) or `SUGU_TOKEN_ENC_PASSPHRASE` to use the AES-256-GCM `FileTokenStore`. Without either the server falls back to the in-memory store and logs a warning at startup. For Cloud Run, inject the key from Secret Manager via the runtime service account; never put it in plain Cloud Run env vars in production.
